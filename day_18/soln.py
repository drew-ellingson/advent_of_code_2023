...
class Grid:
    def __init__(self, points):
        self.points = points
        self.min_y = min(x.pos[0] for x in self.points)
        self.max_y = max(x.pos[0] for x in self.points) + 1

        self.min_x = min(x.pos[1] for x in self.points)
        self.max_x = max(x.pos[1] for x in self.points) + 1
    ...
    def get_enclosed_area(self):
        ...
        for i in range(self.min_y, self.max_y):
            for j in range(self.min_x, self.max_x):
                if any(p.pos == (i, j) for p in self.points) or poly.contains(
                    s_point((i, j))
                ):
                    interior.append((i, j))
